...
abstract class TweetSet {
   ...
   */
   def union(that: TweetSet): TweetSet = {
    filterAcc(tw => !that.contains(tw), that)
  }
...
class Empty extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    throw new java.util.NoSuchElementException("Tweet list is empty")
  }
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweetedAcc(acc: Tweet) = {
    val acc1 = if (acc.retweets < elem.retweets) elem else acc
    val acc2 = left.mostRetweetedAcc(acc1)
    right.mostRetweetedAcc(acc2)
  }

  def descendingByRetweet() : TweetList = {
    val tweet = mostRetweeted
    val remainder = remove(tweet)
    new Cons(tweet, remainder.descendingByRetweet)
  }

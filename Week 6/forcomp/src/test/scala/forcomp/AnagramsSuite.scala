...
class AnagramsSuite extends FunSuite {
  ...
  test("dictionaryByOccurrences: xxxxx") {
    assert(dictionaryByOccurrences.contains(List(('x', 5))) === false)
  }

  test("dictionaryByOccurrences: List()") {
    assert(dictionaryByOccurrences.contains(List()) === false)
  }
  ...
  test("subtract: l - l") {
    val l = List(('l', 1))
    assert(subtract(l, l) === List())
  }

  test("subtract: l - empty") {
    val l = List(('l', 1))
    assert(subtract(l, List()) === l)
  }
